...
# 验证关键模块能导入（PYTHONPATH 指向 gpm-web-admin，否则找不到 app 包）
log "  验证模块导入..."
( export PYTHONPATH="$DEPLOY_DIR/gpm-web-admin" && "$VENV_DIR/bin/python" -c "from app.main import app; print('  导入验证通过')" 2>&1 ) \
